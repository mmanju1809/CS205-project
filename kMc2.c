  ...
  for (i = 0; i < IT; i++)
    {
      if (k[i] != 4)
	{
	  printf("%Lf %Lf %Lf %f\n",llocs[y][0][i]*1e9,llocs[y][1][i]*1e9,llocs[y][2][i]*1e9,tt);
	  E++;
	}
      else
	{
...
int main(int argc, char** argv)
{
  ...
  for (ii = 0; ii < IT; ii++)
    {
      llocs[0][0][ii] = hlocs[ii][0];//10.0e-10*((1.0*((rand()%(RAND_MAX-1))+1))/(1.0*RAND_MAX)-0.5);//;
      llocs[0][1][ii] = hlocs[ii][1];//10.0e-10*((1.0*((rand()%(RAND_MAX-1))+1))/(1.0*RAND_MAX)-0.5);//hlocs[ii][1];
      llocs[0][2][ii] = hlocs[ii][2];//10.0e-10*((1.0*((rand()%(RAND_MAX-1))+1))/(1.0*RAND_MAX)-0.5);//hlocs[ii][2];
    } 
  frac = 0.0;
  for (i = 0; i<ITER; i++) 
    {
