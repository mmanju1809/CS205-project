...
/* Run simulation */
void BFS(long double A[P_SIZE][5], long double P[P_SIZE][5],long double P2[P_SIZE][5], int world_rank, int world_size, int m,MPI_Win win) {
...
int main(int argc, char** argv) {
  ...
  MPI_Win_create(swtcher,P_SIZE,sizeof(int),MPI_INFO_NULL,MPI_COMM_WORLD,&win2);
      ...
      if (swtcher2[i] == 0 || swtcher2[i] == 2) {
        for (k = 0; k < 12; k++) {
          ...
          // below plane
      		  ...
      		  for (j = 0; j < 3; j++) {
    		      A[n_index][j] = opvec1[swtcher2[i]][k-6][j];
    		    }
      		  A[n_index][3] = 1./Q1;
      		  A[n_index][4] = r3/Q1;
      		  swtcher[n_index] = (swtcher2[i]+3)%4;
		     /*   if (m < N-1){
              // replaced + pow(12, m+1) - pow(12, m)
      		    width2 = width + (11*pow(12, m));
      		    size2 = world_size < width2 ? world_size : width2;
          ...
          // above plane
      		  ...
      		  for (j = 0; j < 3; j++) {
  	  	      A[n_index][j] = opvec2[swtcher2[i]][k-9][j];
  		      }
      		  A[n_index][3] = 1./Q1;
      		  A[n_index][4] = r4/Q1;
      		  swtcher[n_index] = (swtcher2[i]+1)%4;
		  /* if (m < N-1) {
              // replaced + pow(12, m+1) - pow(12, m)
      		    width2 = width + (11*pow(12, m));
      		    size2 = world_size < width2 ? world_size : width2;
  ...
  MPI_Win_create(P,P_SIZE*5,sizeof(long double),MPI_INFO_NULL,MPI_COMM_WORLD,&win);
  P[0][0] = 0.0;
  P[0][1] = 0.0;
  P[0][2] = 0.0;
  P[0][3] = 0.0;
  P[0][4] = 1.0;
  for (i = 0; i < N+1; i++) {
    BFS(A, P, P2, world_rank, world_size, i,win);
  }
